...
function updateTotal() {
    ...
    if(cart){
        cart.forEach(function (item) {
            ...
            list+='<h5 style="text-transform:uppercase;font-size:13px"><span class="item-quantity">'+item.quantity+'</span>'+item.name+'<span class="pull-right" style="margin-left: 30px;font-weight: bold">'+formatNumber(subt, '.', ',')+' Đ</span></h5>'
        })
        // for (var i = 0; i < cart.length; i++) {  //loop through the array
    ...
    $('.total').text(formatNumber(total, '.', ',')+' Đ');
}
